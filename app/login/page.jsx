...
export default function Login() {
  return (
    <div>
      <section className="w-full h-[100vh] overflow-hidden -z-1 absolute">
        ...
        <div className="bg-secondary max-w-[300px] rounded-2xl overflow-hidden m-auto"> {/* container */}
          <form className="flex flex-col items-center justify-center gap-4 text-center pt-8 px-6 pb-6">
            <div className="flex flex-col gap-4"> {/* head */}
              <span className="font-bold text-black text-2xl">Iniciar Sesión</span>
              <p className="text-[#7C6666] text-lg">Con una cuenta existente.</p>
            </div>
            ...
            <div>
              <a href="#" className="font-light text-[#0066ff] transition-all ease-in-out duration-3000">
                ¿Olvidaste tu contraseña?
              </a>
            </div>
            <button className="bg-primary text-white w-full h-10 pt-2 pb-2 border-0 overflow-hidden rounded-3xl text-base font-semibold cursor-pointer transition-all ease-in-out duration-1000 hover:bg-[#005ce6]">Inicia sesión</button>
          </form>
          <div className="bg-tertiary p-4 text-base text-center"> {/* form-footer */}
            <p>¿No tienes una cuenta? <Link href="/signup" className="font-bold text-[#0066ff] transition-all ease-in-out duration-3000">Crea una.</Link></p>
          </div>
        </div>
      </div>
    </div>
